...
Runs three ways, and needs no account for the first:
  ...
  TULIP_MODEL_URL=http://127.0.0.1:8010 python -m tulip.rogue
      Any OpenAI-chat-compatible server -- vLLM, Ollama, LM Studio,
      llama.cpp. The served model is discovered from the endpoint; override
      with TULIP_MODEL_NAME. This is the most honest mode: a REAL model,
      really talked into it, really stopped. Small open models fold far more
      easily than a frontier one, so the gate does visible work.

  ANTHROPIC_API_KEY=... python -m tulip.rogue
