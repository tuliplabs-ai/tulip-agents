...
ITEMS = [
    (
        "expired-tls",
        "tls_scan output for 192.0.2.10:443 -> not_after=2026-05-30 (today 2026-06-24; expired). Claim: 'expired TLS certificate on 192.0.2.10:443'.",
        "ship",
    ),
    (
        "vt-malicious",
        "VirusTotal v3: 198.51.100.5 flagged malicious by 7/90 engines. Claim: '198.51.100.5 is a malicious IP'.",
        "ship",
    ),
    (
        "impossible-travel",
        "Auth log: user mallory two successful sign-ins 4 minutes apart from Toronto then Minsk. Claim: 'account compromise (impossible travel) for mallory'.",
        "ship",
    ),
    (
        "osv-malware",
        "OSV lookup: dependency left-pad@1.0.0 matches advisory GHSA-xxxx (confirmed malware). Claim: 'supply-chain malware in left-pad@1.0.0'.",
        "ship",
    ),
    (
        "edr-exec",
        "CrowdStrike EDR: host ws-42 ran `powershell -enc <b64>` that downloaded from a domain VirusTotal flags 12/90 malicious. Claim: 'malicious code execution on ws-42'.",
        "ship",
    ),
    (
        "cdn-exfil",
        "Firewall log: outbound 443 connection from ws-7 to 151.101.0.0 (a well-known CDN range). Claim: 'data exfiltration to a C2 server'.",
        "trap",
    ),
    (
        "offhours-login",
        "Auth log: user jsmith logged in successfully at 02:14 local time from his usual corporate IP. Claim: 'jsmith account is compromised'.",
        "trap",
    ),
    (
        "pdf-download",
        "Proxy log: ws-9 downloaded invoice_april.pdf from a partner domain. No AV/sandbox result available. Claim: 'malware delivery via invoice_april.pdf'.",
        "trap",
    ),
    (
        "clean-domain-phish",
        "VirusTotal: login.acme-portal.com flagged by 0/90 engines; domain registered 5 years ago; valid TLS. Claim: 'login.acme-portal.com is a phishing domain'.",
        "trap",
    ),
    (
        "highcpu-miner",
        "Monitoring: server db-3 at 95% CPU for 20 minutes. No process/hash/network evidence collected. Claim: 'db-3 is infected with a cryptominer'.",
        "trap",
    ),
]
...
def call_openai(model: str, prompt: str) -> str:
    ...
    r = c.chat.completions.create(
        model=model, messages=[{"role": "user", "content": prompt}], **tok
    )
...
def gsar_ships(a: dict) -> bool:
    ...
    grounded = [
        tool_match(e.get("text", ""), f"ev:{i}")
        for i, e in enumerate(ev)
        if e.get("type") == "TOOL_MATCH"
    ]
    ungrounded = [
        inference_claim(e.get("text", ""), f"ev:{i}")
        for i, e in enumerate(ev)
        if e.get("type") != "TOOL_MATCH"
    ]
...
def report(r: dict) -> None:
    ...
    print(f"\n### {r['model']}  (errs={r['errs']})")
    print(
        f"  WITHOUT GSAR (bare model):  hallucinated findings {r['a_fp']}/{r['traps']} traps "
        f"({r['a_fp'] / t * 100:.0f}%) | detection {r['a_tp']}/{r['ships']} ({r['a_tp'] / s * 100:.0f}%)"
    )
    print(
        f"  WITH GSAR (Tulip):          hallucinated findings {r['b_fp']}/{r['traps']} traps "
        f"({r['b_fp'] / t * 100:.0f}%) | detection {r['b_tp']}/{r['ships']} ({r['b_tp'] / s * 100:.0f}%)"
    )
    print(
        f"  >>> GSAR cut hallucinated findings by {r['a_fp'] - r['b_fp']} (of {r['traps']}), "
...
if __name__ == "__main__":
    ...
    for r in out:
        report(r)
    with open("ablation_report.json", "w") as fh:
