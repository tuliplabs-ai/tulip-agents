...
def grounded_finding():
    ...
    part = Partition(
        grounded=[
            tool_match(
                "VirusTotal: 198.51.100.5 flagged malicious by 7/90 engines", "tool:vt:malicious=7"
            ),
    ...
    return ground_finding(
        ...
        asset="ws-7",
        remediation="Contain ws-7 and block the IP.",
        partition=part,
    )
...
async def main() -> None:
    ...
    print(
        f"finding: grounded (gsar={f.gsar_score:.2f}) | verified (survives={verdict.survives}, "
    ...
    try:
        res = await admit(
            a, lambda: do(a.name, a.asset), policy=policy, finding=f, verdict=verdict, trail=trail
        )
    ...
    try:
        await admit(
            b, lambda: do(b.name, b.asset), policy=policy, finding=f, verdict=verdict, trail=trail
        )
        print("[B] isolate_host EXECUTED  <-- should NOT happen")
    except AdmissionError as e:
        print(
            f"[B] isolate_host (production, blast 50) -> HELD ({e.decision.outcome}): {e.decision.reason}"
        )
        print(
            f"    >>> prod host actually isolated? {executed.get('isolate_host', False)}  "
    ...
    recs = trail.records()
    recs[1] = replace(
        recs[1], payload={**recs[1].payload, "outcome": "allow"}
    )  # forge "HELD"->"allow"
    forged = AuditTrail.from_records(recs)
    print(
        f"forge one decision (require_human -> allow) and re-check: chain intact: {forged.verify()}  "
        "<-- tampering detected"
    )
