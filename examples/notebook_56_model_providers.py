...
"""Notebook 56: Model providers — OpenAI, Anthropic.
...
def example_direct():
    ...
    print(
        f"AI rationale: {_llm_call('In one sentence, when would you instantiate a model class directly instead of via the registry?')}"
    )
...
async def example_live_call() -> None:
    ...
    t0 = _t.perf_counter()
    result = agent.run_sync("Name two strengths of building agents with a single SDK.")
